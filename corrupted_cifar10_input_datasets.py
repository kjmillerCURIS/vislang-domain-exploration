...
class CorruptedCIFAR10ImageInputOneDomainDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, params, image_adapter_input_dict_filename, class_domain_dict_filename, non_target, swap_class_and_domain=False):
        p = params
        self.non_target = non_target
        key_non_target = 'domain'
        if swap_class_and_domain:
            key_non_target = 'class'
        ...
        for image_base in sorted(image_adapter_input_dict.keys()):
            if class_domain_dict[image_base][key_non_target] == self.non_target:
