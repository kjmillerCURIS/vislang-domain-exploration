...
class DisentanglementModel(nn.Module):
    ...
    def __compute_initial_components(self, params, num_classes, num_domains, embedding_size, dataset, backbone, num_workers=0):
        p = params
    ...
    def __init__(self, params, num_classes, num_domains, embedding_size, dataset=None, backbone=None, num_workers=0):
        super().__init__()
        p = params
        if dataset is not None and backbone is not None:
            class_components, domain_components, offset_component = self.__compute_initial_components(p, num_classes, num_domains, embedding_size, dataset, backbone, num_workers=num_workers)
    ...
    def forward(self, embeddings, gt_classes, gt_domains):
        ...
        loss = loss.half() #so it can play nicely with the (currently) fp16 CLIP loss
        return loss
